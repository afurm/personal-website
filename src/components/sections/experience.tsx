...
import { FaCode, FaChartLine, FaLaptopCode, FaServer, FaChartBar, FaChild, FaBaby } from "react-icons/fa";
...
const experiences = [
    {
        ...
        period: "2021 - Present",
        description: "Working on blockchain and Web3 technologies, contributing to the development of decentralized applications and infrastructure.",
    ...
    },
    {
        ...
        role: "Senior Software Engineer",
        period: "2019 - 2021",
        description: "Built and launched a career management advisory UI for a fintech startup, enabling executives to manage career paths more proactively—received 90%+ positive user feedback.",
    ...
    },
    {
        ...
        period: "2017 - 2019",
        description: "Led technical decision-making in multiple projects, ensuring scalable, maintainable, and high-performance architecture for banking and finance applications.",
    ...
    },
    {
        ...
        description: "Welcomed my child into the world and began the most rewarding journey of my life.",
    ...
    },
    {
        ...
        period: "2015 - 2017",
        description: "Developed full-stack solutions for startups and career management platforms, focusing on responsive design and user experience.",
    ...
    },
    {
        ...
        description: "Crafted a Ruby DSL for healthcare report rendering, turning raw data into actionable charts and tables. Tech: Ruby, Rails, RSpec, Cucumber",
                    ...
                    {experiences.map((exp, index) => (
                        ...
                        >
                            ...
                            <div className="md:hidden flex flex-row items-start">
                                ...
                                <div className="relative">
                                    <div className={`flex items-center justify-center w-12 h-12 rounded-full ${exp.highlight ? 'bg-pink-100 border-pink-300 dark:bg-pink-950 dark:border-pink-800' : 'bg-card border-border'} border shadow-sm z-10`}>
                                        {exp.icon}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 ml-6">
                                    <div className={`rounded-lg border ${exp.highlight ? 'border-pink-300 bg-pink-50 dark:bg-pink-950/20 dark:border-pink-800' : 'border-border bg-card'} p-6 shadow-sm ${exp.highlight ? 'ring-2 ring-pink-300 dark:ring-pink-800' : ''}`}>
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className={`text-xl font-bold ${exp.highlight ? 'text-pink-600 dark:text-pink-400' : ''}`}>{exp.company}</h3>
                            ...
                            <div className={`hidden md:block w-full md:w-5/12 ${exp.side === "right" && "md:order-1"}`}>
                                <div className={`rounded-lg border ${exp.highlight ? 'border-pink-300 bg-pink-50 dark:bg-pink-950/20 dark:border-pink-800' : 'border-border bg-card'} p-6 shadow-sm ${exp.highlight ? 'ring-2 ring-pink-300 dark:ring-pink-800' : ''}`}>
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className={`text-xl font-bold ${exp.highlight ? 'text-pink-600 dark:text-pink-400' : ''}`}>{exp.company}</h3>
                                        <span className="text-sm text-muted-foreground">{exp.period}</span>
                            ...
                            <div className={`hidden md:flex absolute left-1/2 transform -translate-x-1/2 items-center justify-center w-12 h-12 rounded-full ${exp.highlight ? 'bg-pink-100 border-pink-300 dark:bg-pink-950 dark:border-pink-800' : 'bg-card border-border'} border shadow-sm z-10`}>
                                {exp.icon}
                            </div>
                        </motion.div>
                    ))}
